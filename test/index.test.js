...
describe('index test', () => {
    ...
    it('parses a valid yaml using a parent template with order', () =>
        validator(loadData(VALID_ORDER_TEMPLATE_PATH), templateFactoryMock)
            .then((config) => {
                assert.isObject(config);
    ...
    it('parses a valid yaml using a parent template with order and wrong teardown', () =>
        validator(loadData(VALID_ORDER_WRONG_TEARDOWN_TEMPLATE_PATH), templateFactoryMock)
            .then((config) => {
                assert.isObject(config);
                assert.deepEqual(config,
                    JSON.parse(loadData('valid_order_and_wrong_teardown_template.json')));
            })
    );

    it('parses a valid yaml with invalid order', () =>
        validator(loadData(INVALID_ORDER_TEMPLATE_PATH), templateFactoryMock)
            .then((config) => {
                assert.isObject(config);
    ...
    it('parses a valid yaml using a parent template with order and warnings', () =>
        validator(loadData(VALID_ORDER_WITH_WARNINGS_PATH), templateFactoryMock)
            .then((config) => {
                assert.isObject(config);
                assert.deepEqual(config,
                    JSON.parse(loadData('valid_order_and_warnings_template.json')));
            })
    );
        ...
        validator(loadData(BAD_STRUCTURE_TEMPLATE_PATH))
            .then((result) => {
                assert.deepEqual(result.template,
                    JSON.parse(loadData('bad_structure_template.json')));
